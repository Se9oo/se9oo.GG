...
const MatchDetailTotal = ({ match }) => {
  ...
  // 검색 소환사 id
  const findSummonerParticipantId = allSummonerGameInfo.find((summoner) => summoner.summonerName == summonerName).participantId;
  // 검색 소환사 게임 정보
  const findSummonerGameInfo = allSummonerGameInfo.find((summoner) => summoner.participantId === findSummonerParticipantId);
  // 검색 소환사 속한 팀 정보
  const summonerTeam = match.teams.find((team) => team.teamId === findSummonerGameInfo.teamId);
  // 상대팀 정보
  const enemyTeam = match.teams.find((team) => team.gameId !== findSummonerGameInfo.teamId);
  // 검색 소환사 속한 팀원 게임 정보
