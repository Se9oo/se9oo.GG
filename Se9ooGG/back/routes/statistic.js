...
router.get('/statistic/loadSummoner', async (req, res, next) => {
  ...
  try {
     if (summonerName) {
      ...
      // match 정보
      const allMatchesInfo = await axios.get(`https://kr.api.riotgames.com/lol/match/v4/matchlists/by-account/${accountId}?endIndex=10&beginIndex=0&api_key=${process.env.API_KEY}`);
