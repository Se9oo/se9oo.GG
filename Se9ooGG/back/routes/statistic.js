  ...
  try {
    if (summonerName) {
      ...
      const allMatchesInfo = await axios.get(
        `https://kr.api.riotgames.com/lol/match/v4/matchlists/by-account/${accountId}?endIndex=10&beginIndex=0&api_key=${process.env.API_KEY}`
      );

      const matchData = await Promise.all(
        allMatchesInfo.data.matches.map(async (match) => {
          const matchInfo = await axios.get(
            `https://kr.api.riotgames.com/lol/match/v4/matches/${match.gameId}?api_key=${process.env.API_KEY}`
          );
          // timeLines 정보
          const matchTimeLines = await axios.get(
            `https://kr.api.riotgames.com/lol/match/v4/timelines/by-match/${match.gameId}?api_key=${process.env.API_KEY}`
          );
    ...
    res
      ...
      .json(err.response.data.status.message);
...
// 사용자 인게임 정보 가져오기
router.get(
  '/statistic/loadSummonerInGame/:summonerName',
  async (req, res, next) => {
